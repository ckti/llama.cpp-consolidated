...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext(
    ...
    if (has_turbo && !use_kv_f16) {
        snprintf(base, 256, "kernel_flash_attn_ext_k%s_v%s_dk%d_dv%d",
                 ggml_type_name(op->src[1]->type), ggml_type_name(op->src[2]->type), dk, dv);
    } else {
...
ggml_metal_pipeline_with_params ggml_metal_library_get_pipeline_flash_attn_ext_vec(
    ...
    const bool has_turbo = op->src[1]->type == GGML_TYPE_TURBO2_0 || op->src[1]->type == GGML_TYPE_TURBO3_0 || op->src[1]->type == GGML_TYPE_TURBO4_0 ||
                           op->src[2]->type == GGML_TYPE_TURBO2_0 || op->src[2]->type == GGML_TYPE_TURBO3_0 || op->src[2]->type == GGML_TYPE_TURBO4_0;
    if (has_turbo && !use_kv_f16) {
        snprintf(base, 256, "kernel_flash_attn_ext_vec_k%s_v%s_dk%d_dv%d%s",
                 ggml_type_name(op->src[1]->type), ggml_type_name(op->src[2]->type), dk, dv, qne_suffix);
    } else {
