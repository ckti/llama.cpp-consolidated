...
ggml_tensor * llm_graph_context::build_moe_ffn(
    ...
    if (hparams.n_expert_used(il) == 1) {
        ...
        ggml_tensor * used_experts = ggml_view_2d(ctx0, experts, n_embd, hparams.n_expert_used(il), experts->nb[1], 0);
        ggml_tensor * experts_pe = ggml_cont(ctx0, ggml_permute(ctx0, used_experts, 1, 0, 2, 3));
        ggml_tensor * summed     = ggml_sum_rows(ctx0, experts_pe); // [1, n_embd, 1]
        moe_out = ggml_reshape_2d(ctx0, summed, n_embd, n_tokens);
    } else {
        // note: use hparams.n_expert_used(il) (not n_expert_used) so warmup stays small
